...
def eval_fn(state):
    utility = 0
    utility += getConsecutive(state,'du')
    utility += getConsecutive(state,'dd')
    utility += getConsecutive(state,'h')
    utility += getConsecutive(state,'v')
    return utility


def getConsecutive(state,direction):
    x = y = 0
    if direction == 'h' or direction == 'du' or direction == 'dd':
        x = 1
    if direction == 'v' or direction == 'du':
        y = 1
    if direction == 'dd':
        y = -1
    ...
    for k,v in state.board.items():
        ...
        while True:
            ...
            if temp in state.board and state.board[temp] == v:
                consecutive += 1
                if consecutive == 3:
                    consecutive += 100
            else:
...
def query_player(game, state):
    ...
    game.display(state)
    column = -1
    print game.legal_moves(state)
    try:
        column= int(raw_input('Your move column? '))
    except ValueError:
        print 'Must be an integer'
    while (True):
        for i in game.legal_moves(state):
            if column == i[0]:
                return i
        print 'This value does not correspond to an open column'
        try:
            column= int(raw_input('Your move column? '))
        except:
            print 'Must be an integer'
...
def alphabeta_player(game, state):
    print 'a'
    return alphabeta_search(state, game,4,None,eval_fn)
...
game = ConnectFour()
play_game(game,query_player,alphabeta_player)
